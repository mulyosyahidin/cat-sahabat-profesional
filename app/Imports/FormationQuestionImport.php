...
class FormationQuestionImport implements ToCollection, WithHeadingRow
{
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $row) {
            ...
            try {
                if (empty($row['jabatan']) || empty($row['jenis_soal']) || empty($row['soal']) || empty($row['kunci'])) {
                    continue;
                }
                ...
                $errorRows[] = $rowNumber + 1;
            }
        }

        if (!empty($errorRows)) {
            throw new \Exception("Terjadi kesalahan pada baris: " . implode(', ', $errorRows));
        }
    }
